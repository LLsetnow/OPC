...
| 抖音/X 视频下载 | `opc media download "URL"` | 下载为 MP4 |
...
**实例状态与连接失败排查**：云扉实例前面是 APISIX 网关。实例一旦离开运行中（`2`），网关会撤销路由并对所有 ComfyUI 路径返回 `404 Route Not Found`，但 OpenAPI 仍会回填旧的服务域名 —— 所以“拿得到地址”不代表“连得上”。`--status` 现在直接显示状态含义（`运行中(2)` / `停止中/网关不可用(22)` 等），`--run` 会在上传任何文件之前先探活，报错里带上网关的真实原因。

若报错提示代理相关（`代理连接失败`、`连接被对端重置`），加 `--no-proxy` 让请求忽略 shell 的 `HTTP_PROXY` / `ALL_PROXY` 直连云扉；也可以设 `AIGATE_NO_PROXY=1`。单个输出文件默认最大下载 512 MB，可用 `--max-download-mb` 调整。
