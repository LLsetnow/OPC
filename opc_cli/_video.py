...
抖音 / X 等平台的下载流程完全一致：yt-dlp 取最佳画质封装为 MP4，并把来源
信息写入元数据。这里集中实现，各平台模块只需提供平台名与专属错误类型。
"""
...
def ensure_ytdlp(error_cls: type[Exception]) -> None:
...
def metadata_arguments() -> list[str]:
...
def find_downloaded_mp4(stdout: str, output_dir: Path) -> Path | None:
...
def write_finder_comment(mp4_path: str) -> bool:
...
def download_video(
    ...
    *,
    ...
    """下载单个视频并返回最终 MP4 的绝对路径。

    ``source_label`` 仅用于日志（如「抖音」「X」）；``error_cls`` 是平台专属的
    异常类型；``failure_hint`` 会在下载失败时追加到错误信息（如 X 的登录提示）。
    """
    ensure_ytdlp(error_cls)
    ...
    print(f"下载{source_label}视频: {url}")
    result = subprocess.run(command, capture_output=True, text=True)
    if result.returncode != 0:
        detail = (result.stderr or result.stdout).strip() or "未知错误"
        message = f"yt-dlp 下载失败: {detail}"
        if failure_hint:
            message = f"{message}\n{failure_hint}"
        raise error_cls(message)

    downloaded = find_downloaded_mp4(result.stdout, destination)
    if downloaded is None:
        raise error_cls(
            f"下载完成但未在 {destination} 找到 MP4 文件，请检查 yt-dlp 输出。"
        )
    if write_finder_comment(str(downloaded)):
